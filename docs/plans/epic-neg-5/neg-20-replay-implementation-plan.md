...
| Retention guard | At start, per stream: `XINFO STREAM` → **oldest retained id** (`first-entry`); requested start `<` it ⇒ throw `ReplayRetentionException` (new, core, unchecked) before delivering anything. Re-checked after every batch read; the oldest retained id advancing past the cursor mid-replay completes `done()` exceptionally. Missing/empty stream: fatal for `at()`/`offset()` starts, contributes zero events for `earliest()` | **Correction to the original plan:** `max-deleted-entry-id` is *not* the trim signal — verified against the compose Redis, `XTRIM MINID`/`MAXLEN` (this system's only trimming, via `StreamTrimmer` and the publisher's `XADD MAXLEN ~`) leave it at `0-0`; only `XDEL` moves it, and nothing here calls `XDEL`. The oldest retained id (`first-entry`) is the field that actually tracks trimming. It cannot distinguish "trimmed away" from "never populated" — both put the requested start below the oldest id — so both fail loudly, which honors the "never silent partial data" rule. `earliest()` *means* "oldest retained", so it is exempt by definition. The mid-replay check closes the gap the trimmer can open under a 12h window: better to abort a backtest than let it silently compute across a hole. |
...
- **Start check**, per stream, on the caller's thread before anything is delivered: `XINFO STREAM` → oldest retained id (`first-entry`, folded from the flat field list). Start ID `<` oldest retained ⇒ `ReplayRetentionException` naming the stream, the requested start and the oldest retained ID. Missing key (`ERR no such key`) or an empty stream: fatal for `at()`/`offset()` starts, an empty contribution for `earliest()`. *(Original plan said `max-deleted-entry-id`; that field only moves on `XDEL`, which this system never issues — see the decision table correction.)*
- **Per-batch check**, after each `XRANGE` returns and before its entries dispatch: re-read the oldest retained id; if it has advanced past this stream's cursor, entries between cursor and batch have been trimmed — complete `done()` exceptionally with `ReplayRetentionException`. The check runs after the read so a trim can never invalidate entries already fetched.
...
- **`max-deleted-entry-id` does not track trimming (original-plan correction).** It moves only on `XDEL`; `XTRIM MINID`/`MAXLEN` — this system's only trimming — leave it at `0-0`, verified against the compose Redis. The guard therefore keys off the **oldest retained id** (`first-entry` from `XINFO STREAM`), the field that actually advances when the trimmer sweeps. Its known limitation — a start below the oldest id could have been trimmed *or* never populated — is resolved by failing loud in both cases (never silent partial data). This still needs Redis ≥ 7.0 `XINFO STREAM` semantics (the docker-compose image qualifies).
- **`XTRIM ~` is approximate.** Entries older than MINID can survive a sweep, so "compute the window edge and compare" mis-reports both ways. Never do retention math from `RetentionPolicy` windows at replay time; read the actual oldest retained id from `XINFO STREAM`.
